...
export const loginController = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const userResult = await cnt.request()
      .input('email', email)
      .query('SELECT id, email, display_name FROM [dbo].[app_user] WHERE email = @email');
    if (userResult.recordset.length === 0) {
      return res.status(401).json({ message: 'Email not found' });
    }
    const { id: user_id, display_name, email: user_email } = userResult.recordset[0];
    // ดึง password hash
    const secretResult = await cnt.request()
      .input('user_id', user_id)
      .query('SELECT password_hash FROM [dbo].[user_secret] WHERE user_id = @user_id');
    ...
    // ดึง role
    const roleResult = await cnt.request()
      .input('user_id', user_id)
      .query('SELECT role FROM [dbo].[user_role] WHERE user_id = @user_id');
...
export const signupController = async (req: Request, res: Response) => {
  ...
  if (result && typeof result === 'object' && 'message' in result && result.message !== 'Register success') {
