...
export const signup = async (email: string, handle: string, display_name: string, password: string, role?: string) => {
  if (!email || !handle || !display_name || !password) return { message: 'Missing fields' };
  try {
    ...
    try {
      ...
      const dupCheck = await transaction.request()
        ...
        .query('SELECT [email], [handle] FROM [dbo].[app_user] WHERE [email]=@email OR [handle]=@handle');
      ...
      const userResult = await transaction.request()
        ...
        .query('INSERT INTO [dbo].[app_user] ([email], [handle], [display_name]) OUTPUT INSERTED.id VALUES (@email, @handle, @display_name)');
      ...
      await transaction.request()
        ...
        .query('INSERT INTO [dbo].[user_secret] ([user_id], [password_hash]) VALUES (@user_id, @hashed_password)');

      // Insert role into user_role (default 'user' if not provided)
      await transaction.request()
        .input('user_id', NVarChar, user_id)
        .input('role', VarChar, role || 'user')
        .query('INSERT INTO [dbo].[user_role] ([user_id], [role]) VALUES (@user_id, @role)');
